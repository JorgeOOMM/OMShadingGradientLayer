...
@objc class OMTextLayer: CALayer {
    ...
    //
    fileprivate func setParagraphStyleSettings(_ newString: CFMutableAttributedString?, _ range: CFRange) {
        // CTParagraph
        let valueSize = MemoryLayout<CGFloat>.size
        let setting: [CTParagraphStyleSetting] = withUnsafeBytes(of: alignment) { alignmentBP in
            withUnsafeBytes(of: lineSpacing) { lineSpacingBP in
                withUnsafeBytes(of: lineBreakMode) { lineBreakModeBP in
                    withUnsafeBytes(of: minimumLineHeight) { minimumLineHeightBytes in
                        withUnsafeBytes(of: maximumLineHeight) { maximumLineHeightBytes in
                            [
                                CTParagraphStyleSetting(spec: .alignment,
                                                        valueSize: MemoryLayout.size(ofValue: alignment),
                                                        value: alignmentBP.baseAddress!),
                                CTParagraphStyleSetting(spec: .lineBreakMode,
                                                        valueSize: MemoryLayout.size(ofValue: lineBreakMode),
                                                        value: lineBreakModeBP.baseAddress!),
                                CTParagraphStyleSetting(spec: .lineSpacingAdjustment,
                                                        valueSize: valueSize,
                                                        value: lineSpacingBP.baseAddress!),
                                CTParagraphStyleSetting(spec: .minimumLineHeight,
                                                        valueSize: valueSize,
                                                        value: minimumLineHeightBytes.baseAddress!),
                                CTParagraphStyleSetting(spec: .maximumLineHeight,
                                                        valueSize: valueSize,
                                                        value: maximumLineHeightBytes.baseAddress!)
                            ]
                        }
                    }
                }
    ...
    func attributedStringWithAttributes(_ attrString: CFAttributedString) -> CFAttributedString {
        ...
        setParagraphStyleSettings(newString, range)
        CFAttributedStringSetAttribute(newString,
                                       range,
...
extension OMTextLayer {
    ...
    func getRunFont(_ run: CTRun) -> CTFont {
        ...
        let runFont = dict.object(forKey: String(kCTFontAttributeName)) as! CTFont  // swiftlint:disable:this force_cast
        return runFont
    }
    func createPathFromStringWithAttributes() -> UIBezierPath? {
        ...
        if let line = createLine() {
            ...
            let run: CTRun = runArray[0] as! CTRun   // swiftlint:disable:this force_cast
...
extension OMTextLayer {
    func prepareGlyphArcInfo(line: CTLine, glyphCount: CFIndex, angle: Double) -> [GlyphArcInfo] {
        ...
        for run in runArray {
            let runGlyphCount = CTRunGetGlyphCount(run as! CTRun)  // swiftlint:disable:this force_cast
            // Ask for the width of each glyph in turn.
            for runGlyphIndex in 0 ..< runGlyphCount {
                let glyphArcInfoIndex = runGlyphIndex + glyphOffset
                let runGlyphRange       = CFRangeMake(runGlyphIndex, 1)
                // swiftlint:disable:next force_cast
                let typoBounds = CTRunGetTypographicBounds(run as! CTRun, runGlyphRange, nil, nil, nil)
                let runTypographicWidth = CGFloat(typoBounds)
                let newGlyphArcInfo     = GlyphArcInfo(width: runTypographicWidth, angle: 0)
                glyphArcInfo.insert(newGlyphArcInfo, at: glyphArcInfoIndex)
            }
    ...
    func drawWithArc(context: CGContext, rect: CGRect) {
        ...
        if let string = string, let angle = self.angleLenght {
            ...
            if glyphArcInfo.count > 0 {
                ...
                // The angle offset for each glyph relative to the previous glyph has already been
                // calculated with that information in hand,
                // draw those glyphs overstruck and centered over one another, making sure
                // to rotate the context after each glyph so the glyphs are spread
                // along a semicircular path.
                ...
                for runIndex: CFIndex in 0 ..< runCount {
                    ...
                    for runGlyphIndex: CFIndex in 0 ..< runGlyphCount {
                        ...
                        let posForThisGlyph: CGPoint = CGPoint(x: textPosition.x - halfGlyphWidth, y: textPosition.y)
                        ...
                        var textMatrix = CTRunGetTextMatrix(run as! CTRun)   // swiftlint:disable:this force_cast
                        textMatrix.tx = posForThisGlyph.x
                        textMatrix.ty = posForThisGlyph.y
                        context.textMatrix = textMatrix
                        CTRunDraw(run as! CTRun, context, glyphRange)   // swiftlint:disable:this force_cast
                    }
                    glyphOffset += runGlyphCount
                }
                context.restoreGState()
